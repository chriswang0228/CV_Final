...
class DroneDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = cv2.imread(self.mask_path + self.X[idx][:-4]+'.png', 0)
        label[ label > 0 ] = 1  
...
t_train = A.Compose([A.Resize(height, width, interpolation=cv2.INTER_NEAREST), A.HorizontalFlip(),
                     A.RandomBrightnessContrast((0,0.5),(0,0.5)),  A.GridDistortion(p=0.2), A.GaussNoise()])
...
n_classes=2
model = smp.create_model(arch=cfg.arch,encoder_name=cfg.encoder, encoder_weights='imagenet', classes=n_classes, activation=None, encoder_depth=5, decoder_channels=[256, 128, 64, 32, 16])
...
def fit(epochs, model, train_loader, val_loader, criterion, criterion_lov, optimizer, scheduler, patch=False):
    ...
    history = {'train_loss' : train_losses, 'val_loss': test_losses,'train_iou0':train_miou0,'train_iou1':train_miou1,
              'val_miou0':val_miou0,'val_miou1':val_miou1, 'train_acc':train_acc, 'val_acc':val_acc
               }
...
loss3 = nn.CrossEntropyLoss(weight=class_weights3)
